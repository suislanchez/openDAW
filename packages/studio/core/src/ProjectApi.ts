...
import {AudioUnitBoxAdapter, RootBoxAdapter, TrackType} from "@opendaw/studio-adapters"
...
export class ProjectApi {
    ...
    createInstrument({create, defaultIcon, defaultName, trackType}: InstrumentFactory,
                     {name, icon}: InstrumentOptions = {}): InstrumentProduct {
        ...
        const audioUnit = this.#createAudioUnit(AudioUnitType.Instrument)
        const audioUnitAdapter = boxAdapters.adapterFor(audioUnit, AudioUnitBoxAdapter)
        const uniqueName = Strings.getUniqueName(existingNames, name ?? defaultName)
        const iconSymbol = icon ?? defaultIcon
        const instrument = create(boxGraph, audioUnitAdapter, uniqueName, iconSymbol)
        const track = TrackBox.create(boxGraph, UUID.generate(), box => {
            box.index.setValue(0)
            box.type.setValue(trackType)
            box.tracks.refer(audioUnitAdapter.tracksField)
            box.target.refer(audioUnitAdapter.audioUnitBoxAdapter().box)
        })
        userEditingManager.audioUnit.edit(audioUnit.editing)
        return {audioUnit, instrument, track}
    }
